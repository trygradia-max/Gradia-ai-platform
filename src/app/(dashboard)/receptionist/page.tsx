      ...
      <header className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
        <div className="space-y-2">
          <p className="label-eyebrow text-muted-foreground/70">
            {STRINGS.pages.receptionist.eyebrow}
          </p>
          <h1 className="font-display text-2xl text-foreground">
            {STRINGS.pages.receptionist.title}
          </h1>
      ...
      <div className="space-y-3">
        {groups.map((group) => {
          ...
          return (
            <CapabilityRow
              key={group.title}
              // Rendered element, not the component: only serializable
              // values may cross the server→client boundary.
              icon={<group.icon className="size-4.5" aria-hidden />}
              title={group.title}
              blurb={group.blurb}
              on={on}
