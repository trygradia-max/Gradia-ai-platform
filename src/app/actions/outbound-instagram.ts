...
import { sendInstagramDmApprovalRequest } from "@/lib/slack"
...
export type ProposeInstagramDmResult =
  | { ok: true; pendingId: string }
  | { ok: false; error: string }

export async function proposeOutboundInstagramDm(
  input: z.infer<typeof proposeSchema>
): Promise<ProposeInstagramDmResult> {
  const parsed = proposeSchema.safeParse(input)
  if (!parsed.success) {
    return {
      ok: false,
      error: parsed.error.issues[0]?.message ?? "Invalid IG DM proposal.",
    }
  }
  ...
  if (pendingErr || !pending) {
    return {
      ok: false,
      error: pendingErr?.message ?? "Couldn't queue the IG DM.",
    }
  }
  ...
  try {
    ...
    console.error("[outbound-instagram] Slack approval send failed:", err)
  }
