export const isCanonicalPhotoId = (value: unknown): value is string => typeof value === "string" && /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/.test(value)
...
export function isOwnedJobPhotoPath(path: unknown, shopId: string, appointmentId: string, phase: "before" | "after"): path is string {
  if ((phase !== "before" && phase !== "after") || typeof path !== "string") return false
