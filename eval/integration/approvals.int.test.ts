...
describe.skipIf(!INTEGRATION)("approval engine [integration]", () => {
  ...
  it("rollback: a failing executor returns the action to pending, no partial writes", async () => {
    // charge_customer with a valid email + amount but no Stripe account on the
    // shop fails in the executor *after* the claim — the claim must roll back.
    const before = await countLeads(sb, seed.shopId)
    const id = await stagePending(sb, seed.shopId, seed.ownerId, "charge_customer", {
      customer_name: "X",
