...
The current production freeze overrides execution eligibility: all 35 application
write guards remain active. Approved target autonomy modes are not live activation
grants. User-requested Supabase authentication email is the sole narrow communication
exception; neither Agent actions nor manager/business notifications inherit it.
PR #45 secures login redirects but does not expand permissions.
...
atomic execution boundary. `send-policy.ts` and **merged PR #44** provide shop- and
