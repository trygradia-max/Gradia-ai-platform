...
| D-053 | **Eval gating: nightly scheduled `npm run eval` with failure notification AND a CI path filter that requires the live-model tier when prompt/recipe files change** (Q-06 option "both"). | Founder-approved 2026-09-01 (autorun prep). Unblocks **E01-05**. Live-model evals still do not gate unrelated PRs; the path filter scopes the cost. Nightly failure notification rides the P0-012 alert seam (D-042). |
| D-054 | **Preview + logs before DONE.** No ticket touching a user-visible flow is DONE until the Builder has exercised that flow on the Vercel Preview with a real browser and read `vercel logs` for the deployment; the Reviewer re-walks it before PASS. Tests alone are not acceptance for such tickets. | Founder-directed 2026-09-02 at the Batch-1 close, after the Gmail root-cause miss: UX-001 attributed the founder's "never connects" repro to a display-email predicate split, while the actual cause was `/api/aurinko/webhook` answering 401 to Aurinko's subscription validation ping (fixed PR #34 `cdb0c99`). Recorded as `program/autorun.md` rule 8 + precondition 6 (Vercel CLI login and browser available to the Builder). Amends nothing in `12-definition-of-done.md` — it adds an acceptance step for autorun tickets; DoD F/G still apply. |
