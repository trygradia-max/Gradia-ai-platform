...
- **ADR-003 — Service-role tenant-scoping mechanism (`forShop`)** (`ADR-003-service-role-tenant-scoping.md`) — **proposed** 2026-08-28 by the P0-011 Builder (needs Organizer review + founder sign-off before the migration ticket set TS-1…TS-6 is cut): a thin explicit `forShop(client, shopId)` facade that pre-applies `.eq("shop_id")` and stamps inserts, with a loud `unscoped` escape hatch; Postgres session-variable RLS-for-service-role evaluated and deferred to an E01-era decision (TS-5). Design proof shipped in P0-011: the facade + two converted cron call sites + the tenant-isolation test tier + the service-role importer inventory lock.
