...
- **Status:** **done — 2026-09-02** (Builder `0b4426d` on `auto/batch-1`; Cursor PASS 2026-09-02 — concurrent `SKIP LOCKED` runs disjoint, floor clamp, anon/owner RPC denied; merged to `main` in PR #33 squash `ff66cc9`; one additive migration `20260901120000_provider_events_pruning.sql`, DB-sensitive slot released at close). Retention windows 30 / 90 / 7 stand as shipped pending founder ratification; the first scheduled Production run is confirmed by the founder from `vercel logs` — both in `../program/backlog.md` Batch-1 residuals. _Was:_ **ready** (filed 2026-08-13 at the P0-005 close, satisfying **ADR-001 condition C2** — this ticket must exist before P0-006 enters implementation; unscheduled — the Organizer sequences it. Pilot-scale growth gives long headroom, but pruning must land before P0-006/P0-007 receipt volume makes the table operationally significant. **Note 2026-08-14 at the P0-007 close: both consumer routes — Twilio inbound (P0-006, PR #19) and Vapi end-of-call (P0-007, PR #21) — are now live and writing receipts; this ticket remains open and the sequencing pressure is now real, not hypothetical.**)
