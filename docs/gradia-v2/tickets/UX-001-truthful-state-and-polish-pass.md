...
## Status
**done — 2026-09-02, with a root-cause correction** (Builder `038851c` on `auto/batch-1`; Cursor PASS 2026-09-02; merged to `main` in PR #33 squash `ff66cc9`). The founder's Gmail repro was **not** the display-email split — it was the Aurinko subscription validation ping being rejected with 401 by `/api/aurinko/webhook`, fixed in **PR #34 `cdb0c99`**; founder verified Connected on their own shop 2026-09-02. UX-001's connection-truth, NOT AVAILABLE, ⓘ help and skeleton changes stand. See the Organizer close note at the end of this file; residuals in `../program/backlog.md` Batch-1 residuals. _Was:_ **draft — precondition-gated.** Autorun Batch 1, queue item 3c (after CLEANUP-001, before PERF-001). **Precondition (founder):** Stripe reference screenshots added to `docs/gradia-v2/ui/reference-board.md` with adopt/reject lists (the board's rule: "a reference without both lists is not approved") before this ticket starts. Risk class **standard** (no schema). Founder acceptance: **founder visual review on Preview** (autorun table). Decisions binding: D-025 (no dead controls / fake UI), D-028 (live/beta/planned truth), D-046/D-049 (no nav change), D-052 (Slack copy gone — CLEANUP-001 precedes). No open decision.
...
## Organizer close note — 2026-09-02 (root-cause correction)

**What the founder actually hit.** The repro in scope item (a) — "Gmail connected via Aurinko, Email card still says Connect Gmail" — was traced by the Builder to the display-email predicate split above. That split is real in code, but it was **not** the cause in the founder's case. The connection had **never persisted**: Aurinko verifies a new subscription's `notificationUrl` by POSTing `?validationToken=<random>` and requires the token echoed back as `text/plain` 200 within 30 s. `/api/aurinko/webhook` ran signature verification on every request and answered **401**, so `createMessagesSubscription` threw `subscription_failed` inside the OAuth callback and no credential row was written — the Aurinko dashboard showed 6 accounts with tokens and 0 subscriptions. Home read "Live" only where a stale/partial row existed; on the founder's shop every surface was truthfully disconnected because nothing had been stored.

**The fix** — PR #34 `cdb0c99` (`fix/aurinko-webhook-validation`, merged to `main` 2026-09-02 ahead of #33): the validation echo runs before signature verification, reads no body and touches no DB; real notifications (no `validationToken`) still require a valid `X-Aurinko-Signature`; an empty token is not treated as a ping. Locked in `eval/webhooks.test.ts` (+4: ping echoes with no side effects; unsigned / empty-token / forged-signature notifications still 401; a correctly signed notification still reaches the shop lookup). **Founder-verified:** Connected on the founder's own shop, 2026-09-02.

**What stands from this ticket.** `connectionStatus()` as the one predicate set (credentials decide; identity is display-only), `integrationAvailability()` as the one env-presence source, the NOT AVAILABLE state replacing "Coming soon", the ⓘ help lines, the five `loading.tsx` files, the parity and source-scan tests. The regression tests named in the root-cause statement remain valid for the split they cover.

**Why the miss happened, and the rule it produced.** The Builder had no browser and no `vercel logs` and reasoned from code + fixtures; the Reviewer confirmed the theory against the code rather than the live connect flow. Both followed the rules as written. `../program/autorun.md` **rule 8** (D-054) now requires any ticket touching a user-visible flow to be exercised on the Vercel Preview with a real browser, with `vercel logs` read, before it is DONE — tests alone are not acceptance.

**Manual acceptance step 4 disposition.** Connection truth on the founder's shop: PASS (after #34). The route-by-route comparison against the reference-board adopt list was not separately recorded at close; it rides the PERF-001 Preview pass (`../program/backlog.md` Batch-1 residuals item 7).
