...
| P0-008 | Twilio subaccount status callback repair | E00 | **blocked — next implementation position** (moved up 2026-08-14 at P0-007 close; blocked only until the `docs/close-p0-007` planning closeout lands on `main` — the Organizer flips this to ready on merge. Entry in `../program/blocked.md`. Its scope now also carries the P0-006-deferred status-callback findings listed in `../program/backlog.md` Band 2) | Medium | P0-002 (done) · closeout merge | none | `P0-008-twilio-subaccount-status-callback-repair.md` |
