...
| P0-005A | provider_events retention and pruning | E00 | **ready** (filed 2026-08-13 per ADR-001 C2 — required to exist before P0-006 starts; unscheduled) | Medium | P0-005 (done) | none expected (confirm at slotting) | `P0-005A-provider-events-retention-pruning.md` |
...
| P0-012 | Monitoring alert delivery and incident hooks | E00 | **ready — next implementation position** (promoted 2026-09-01 at the P0-011 close per backlog order; the delivery seam builds now — only the final destination config waits on Q-08 per `../program/blocked.md`; new consumers waiting on it: `TENANT_SCOPE_VIOLATION` signals (P0-011) + the P0-008 M1 credential observability follow-up. Enters in-progress when the Organizer slots it and a Builder is recorded on the WIP board) | Medium | P0-002 (done); decision Q-08 (alert destination — config only) | none | `P0-012-monitoring-alert-delivery-incident-hooks.md` |
| P0-013 | Production billing model alignment (D-031 three-tier implementation) | E00 | **draft — decision-gated** (cut 2026-08-28 at the P0-010 close; may not enter implementation until the founder resolves the Q-22 commercial tier decisions for Core/Pro/Operator. **Launch-blocking before live paid billing activation** — the `STRIPE_PRICE_*` Production env vars stay unset until it is implemented, reviewed, accepted, and ready) | High | Q-22 (founder) · P0-010 (done — recorded the billing exception) | payments + database-sensitive | `P0-013-production-billing-model-alignment.md` |
