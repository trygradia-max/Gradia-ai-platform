...
| P0-005A | provider_events retention and pruning | E00 | **done** (2026-09-02 — autorun Batch 1 item 2, Builder `0b4426d`, merged PR #33 `ff66cc9`; Cursor PASS on local-Postgres falsifications; one additive migration `20260901120000_provider_events_pruning.sql`; 30/90/7 windows Builder-proposed — founder ratification + first scheduled run tracked in `../program/backlog.md` Batch-1 residuals; record in the ticket file) | Medium | P0-005 (done) | database-sensitive (one additive migration; slot released at close) | `P0-005A-provider-events-retention-pruning.md` |
...
| P0-012 | Monitoring alert delivery and incident hooks | E00 | **done — steps 1–5** (2026-09-02 — autorun Batch 1 item 3, Builder `12fe1b0`, merged PR #33 `ff66cc9`; Cursor PASS; `lib/alerts.ts` seam + `runCron` on all nine crons + `GET /api/health` + `cron_heartbeats` migration live; **step 6 — founder sets `OPS_ALERT_WEBHOOK_URL` in Production and confirms one real test alert — outstanding as a founder action in `../program/backlog.md`**; alerts are fail-open console + Sentry until then; record in the ticket file) | Medium | P0-002 (done) · D-042 | database-sensitive (one additive migration; slot released at close) | `P0-012-monitoring-alert-delivery-incident-hooks.md` |
...
|---|---|---|---|---|---|---|---|---|
| PROD-CONFIG-AUDIT | Production configuration audit (docs-only output; incl. the D-036 ICP amendment to `ui/design-north-star.md` + `ui/navigation-model.md`) | E00 | **done** — item 1 (2026-09-02; Builder `0043401`, merged PR #33 `ff66cc9`; Cursor PASS; runbook live at `../runbooks/production-config-audit.md`; founder presence fill outstanding in `../program/backlog.md`) | Medium | none | none | no | `PROD-CONFIG-AUDIT-production-config-audit.md` |
| CLEANUP-001 | Remove the Housecall Pro connector and the Slack approvals surface (D-052; supersedes P3-001; closes ADR-003 TS-6 by removal) | E00 | **done** — item 3b (2026-09-02; Builder `4bd2468` + `ae7423f`, merged PR #33 `ff66cc9`; Cursor PASS, one MEDIUM → backlog; founder owes the six env-var removals) | Medium | P0-012 (alert seam carries payment notices + reconciliation drift) · D-052 | standard | no | `CLEANUP-001-remove-housecall-pro-and-slack-approvals.md` |
| UX-001 | Truthful state + polish pass (connection truth root-cause, stale copy, Stripe-pattern inline help, required states) | E00 | **done, with a root-cause correction** — item 3c (2026-09-02; Builder `038851c`, merged PR #33 `ff66cc9`; Cursor PASS; the founder's Gmail repro was the Aurinko validation-ping 401, fixed in PR #34 `cdb0c99`, founder verified Connected; UX-001's connection-truth changes stand; see the Organizer close note in the ticket file) | High | CLEANUP-001 · PROD-CONFIG-AUDIT | standard | founder visual review on Preview | `UX-001-truthful-state-and-polish-pass.md` |
| PERF-001 | Response-time audit and fixes (measure → fix top causes → re-measure; p75 TTFB < 600 ms on five routes; Approve < 100 ms) | E00 | **ready — next** — item 3d (Batch 1 items 1–3c merged 2026-09-02; fresh branch from `main`; measures on the Vercel Preview per autorun rule 8; also carries the UX-001 reference-board route walk) | Medium-high | UX-001 · PROD-CONFIG-AUDIT | standard (additive indexes = DB slot) | no | `PERF-001-response-time-audit-and-fixes.md` |
