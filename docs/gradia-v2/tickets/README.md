...
| P0-007 | Vapi transcript and usage replay protection | E00 | **blocked — next implementation position** (moved up 2026-08-14 at P0-006 close; blocked only until the `docs/close-p0-006` planning closeout lands on `main` — the Organizer flips this to ready on merge. ADR-001 C3 + C5 bind its scope: claim only AFTER verification, test-locked; explicit route `maxDuration` with `staleAfterSeconds` strictly above it. Entry in `../program/blocked.md`) | High | P0-005 (done) · P0-006 (done) · closeout merge | payment (metering) | `P0-007-vapi-transcript-usage-replay-protection.md` |
