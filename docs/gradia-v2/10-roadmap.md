...
|---|---|---|---|---|
| **P0** | Stabilization | E00 | Fix what makes the current platform leaky: credential leak, CI depth, conflicts, idempotency, broken seams, alerting. | All 12 P0 tickets done; audit re-scores security ≥7 and reliability ≥7 (re-score = a non-Builder session re-runs audit docs 06/10 scoring with cited evidence — never self-certified by the implementing session); alpha shippable (date policy: Q-25). **Scope note (2026-08-28):** P0-013 (billing model alignment) sits in the P0 band but is decision-gated on Q-22 and gates **live paid billing activation / launch**, not this stabilization exit — Production checkout stays fail-closed (`STRIPE_PRICE_*` unset) in the interim, per the P0-010 close record. |
