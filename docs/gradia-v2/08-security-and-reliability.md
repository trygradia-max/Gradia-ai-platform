...
| M-2 | MEDIUM | Agent `config` accepted as `z.unknown()` cast to `AgentConfig`, feeds audience queries/prompts. | Originally slated to ride P0-010, but the cut ticket's scope never included it and **P0-010 shipped without it (2026-08-28)** — still open; backlog follow-up recorded at the P0-010 close (real zod schema) |
