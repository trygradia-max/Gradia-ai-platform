...
|---|---|
| Alpha hardening release(s) | Individual P0-ticket batches may ship to prod as they pass review — flags off where risky. Shipped so far: P0-001 (PR #8, 2026-07-30) · P0-002 (PR #9, 2026-07-30) · P0-003 (PR #10, 2026-08-06) · P0-004 (PR #12, 2026-08-11 — conflict enforcement wired on every path, dormant behind `NEXT_PUBLIC_GRADIA_CONFLICT_ENFORCEMENT`) · P0-004A (PR #15, 2026-08-11 — booking atomicity/concurrency, live immediately regardless of the flag) · P0-005 (PR #17, 2026-08-13 — webhook idempotency foundation; staging manual acceptance still gates full rollout acceptance of its migrations) · **P0-006 (PR #19, 2026-08-14 — Twilio inbound replay hardening: inbound SMS route claims `provider_events` after signature verification; durable classification metering; no new migration; Cursor APPROVE / safe to merge; founder real-Twilio staging acceptance pre-merge; see `../releases/README.md`)** · **autorun Batch 1 items 1–3c (PR #33 `ff66cc9`, 2026-09-02 — PROD-CONFIG-AUDIT docs, P0-005A pruning + migration, P0-012 alert seam + `/api/health` + heartbeats migration, CLEANUP-001 HCP/Slack removal, UX-001 connection truth + help + skeletons)** · **Aurinko validation-ping fix (PR #34 `cdb0c99`, 2026-09-02 — Gmail connections now persist)**. The P0-004 production flag flip remains a release event, gated only on founder manual acceptance (P0-004A hardening gate satisfied). |
...
- **Housecall Pro** — connector **removed** (D-052 / CLEANUP-001, merged 2026-09-02); never claimable unless a new decision reinstates it. Housecall Pro *export files* remain an E03-02 import source.
