...
## Q-26 — Disposition of the 2026-09-04 future-directions ideas (six theses + the business-agent ambition)

- **Context:** Founder filed six long-range directions plus a "Gradia's business agent" capability table in `_docs/IDEAS_NOTES.md` (2026-09-04, verbatim). Reviewed against HEAD `e1f09c6` and `platform/CONTEXT.md`: `../program/idea-review-2026-09-04-future-directions.md`. Findings in short: four of the six describe subsystems that already ship (shared memory `memory.ts`/`knowledge.ts`; the structured-query-vs-RAG split `bi-tools.ts`; Approval Mode as the autonomy control system `approvals.ts`+`trust.ts`+D-068; the agent-interop surface `lib/mcp/server.ts`, 12 tools, hashed bearer tokens, per-shop binding — unmentioned in the entry). The prototype architecture in the source material proposes a **Cursor SDK agent** as the runtime, which duplicates `agent-runtime.ts` (2,154 L) against D-010 and whose own documented behaviour (custom tools execute without interactive approval; restrictions must be reapplied on resume) conflicts with Guardrail #1. Three factual corrections to the entry are recorded in the review: `forShop()` is live at 2 call sites against ~221 raw `.from()` calls in `src/lib` (so "the database enforces access" is aspirational — Guardrail #5); `availability.ts` contains no reference to the service menu and falls back to `DEFAULT_DURATION_MINUTES = 90`, so the "changing a duration cascades" example cascades to nothing today; and multimodal Whisper lands in cut scope per D-067 (`whisper.ts` = 2 intents, zero vision calls in `src/`).
- **Options (founder sets each):**
  1. **Cursor SDK runtime** — adopt / reject / require an ADR against D-010. *Recommendation: **reject**; reopening needs an ADR, not a diagram.*
  2. **B-19 draft/edit delta capture** (`../tickets/B-19-draft-edit-delta-capture.md`) — insert into `CONTEXT.md` §4 after B-04 / defer / reject. *Recommendation: **insert**. It is the only new build this review endorses; it is one session; and the data cannot be backfilled, so deferring destroys signal permanently.*
  3. **Two-shop tenancy red team** (identical customer names, cross-shop probes, duplicate events, an injected customer message) — run against the existing runtime as a **security acceptance run** feeding the P0 re-score, or leave to E01-02's forShop rollout. *Recommendation: **run it**, scoped as acceptance, not as a research prototype; it is the cheapest available test of whether ~221 unscoped sites hold.*
  4. **Duration propagation** — B-16 (merged branch `auto/b-16-0903-2010`) collects per-size durations but nothing downstream of quoting consumes them, and `availability.ts` reads no service data. *Recommendation: **cut a small follow-up ticket** — pass `sizeClass` at the 4 dropping call sites and make `lookupServiceDuration` go through `resolveDurationMinutes`; treat the availability↔services wiring as a separate, larger item for E02. Do not fold either into B-16's second half.*
  5. **Everything else** (operational digital twin / scenario simulation, multimodal Whisper, agent-to-agent commerce, developer platform) — hold. *Recommendation: **hold to a 50-paying-shop trigger**. Ranking rationale in the review: the simulation half of thesis 1 sells enterprise planning to an SMB that answers the question in ten seconds; theses 5 and 6 are network plays and there are zero shops; the MCP server means the cost of entering both later is low, which is a reason to wait rather than to move.*
- **Recommendation (whole item):** adopt 2, 3 and 4; reject 1; hold 5. The strategic read the review endorses: the moat is not "long-running agents" — it is being the only vendor in this vertical that can let agents *act* under D-068's reversibility × blast-radius ladder with non-bypassable consent/quiet-hours/STOP in code, in a $500–$1,500-per-message TCPA environment. That is B-17, already ordered. Nothing in Q-26 should displace B-02 → B-10, or the founder-only A2P Brand registration that currently blocks SMS entirely.
- **Blocks:** nothing in `CONTEXT.md` §4 today. B-19 cannot be built until option 2 is decided (agents do not self-insert tickets into §4, per §8).
