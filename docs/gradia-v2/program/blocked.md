...
| P0-007 Vapi transcript and usage replay protection (next implementation position) | The `docs/close-p0-006` planning closeout landing on `main` — this row leaves the file when that commit lands and the Organizer flips P0-007 to ready. (Predecessor row resolved: the P0-006 unblock conditions landed in PR #18 `def97ab`; P0-006 then implemented and merged 2026-08-14, PR #19 `76847e4` — recorded in its ticket file.) Implementation must honor ADR-001 C3 (claim strictly after verification, test-locked) and C5 (explicit route `maxDuration` with `staleAfterSeconds` strictly above it). | 2026-08-14 | Organizer |
