...
flowchart TD
    ...
    Q22["Q-22 tier decisions (founder)"] --> P0013["P0-013 billing model alignment (launch gate)"]
...
- **P0-011 → E01**: P0-011's `forShop()` helper *design* is an input to the E01 tenancy mechanism. P0-011 itself has no P0 dependencies beyond P0-001/002. **Delivered 2026-09-01 (PR #29 `e02c81a`):** ADR-003 founder-approved — E01's tenancy-mechanism input now exists (facade + two proof conversions + tenant-isolation test tier); the migration batches TS-1…TS-6 and the TS-5 session-variable-RLS design gate are recorded in the ADR for E01-era sequencing.
- **P0-008, P0-009, P0-010, P0-012** are independent of each other and of tracks A/B — schedulable whenever slots free (P0-012's destination consults Q-08 — see `blocked.md` for what may proceed regardless). **P0-008 done 2026-08-25** (PR #23 — no dependency edges changed; it consumed nothing and nothing waits on it except the E07 delivery-chip UI, which now has live data). **P0-009 done 2026-08-26** (PR #25 — Q-04 stayed non-blocking exactly as mapped: the minimal honest expired state shipped, the richer CTA still waits on the decision; E05's quote-linkage prerequisite is now satisfied). **P0-010 done 2026-08-28** (PR #27, reviewed tree `618cf41` — no dependency edges changed; P0-011 promoted to next implementation position at its close). **P0-011 done 2026-09-01** (PR #29 `e02c81a` — the P0-009 soft-ordering edge below closed as designed; **P0-012 promoted to next implementation position** at its close; P0-012's `TENANT_SCOPE_VIOLATION` consumer edge added — the signal is emitted now, delivery lands with P0-012). **P0-012 done 2026-09-02** (PR #33 `ff66cc9` — delivery seam live, `TENANT_SCOPE_VIOLATION` delivers as SEV-0; the founder's destination config, step 6, is the last operational piece). **P0-005A done 2026-09-02** (PR #33) closes the retention branch of the P0-005 graph — only the Aurinko email dedupe follow-up (ADR-001 C4) remains there.
- **P0-013 (Production billing model alignment — cut 2026-08-28 at the P0-010 close):** hard-gated on the founder's **Q-22** tier decisions; consumes P0-010's recorded production billing exception (`STRIPE_PRICE_*` absent = checkout fail-closed) and is **launch-blocking before live paid billing activation**. It shares no code edge with P0-011/P0-012 and blocks neither; it occupies the payments high-risk + database-sensitive WIP slots when it eventually enters, so it cannot run concurrently with P0-011 (tenancy shares the single high-risk slot).
- **P0-009 → P0-011 (soft ordering, added 2026-07-27):** both touch `approvals.ts`; P0-009's quote-linkage fix should merge before P0-011's scoping sweep re-reviews that file, so the sweep reviews final code, not code about to change. **Satisfied 2026-08-26; edge CLOSED 2026-09-01** — the P0-011 sweep re-reviewed final `approvals.ts` and completed the post-P0-009 item: `recordPayloadReconciliation` now carries the `shop_id` predicate on both its read and write (close record in the P0-011 ticket).
...
| P0-012 (destination config) | Q-08 → D-042 | Seam shipped 2026-09-02 (PR #33); founder sets `OPS_ALERT_WEBHOOK_URL` + fires the test alert (`backlog.md` Batch-1 residuals) |
| P0-013 (billing model alignment) | Q-22 | **Hard gate** — nothing enters implementation before the founder's tier decisions are recorded; `STRIPE_PRICE_*` stays unset in Production (checkout fail-closed) in the interim; launch-blocking before live paid billing |
...
8. **Reliable event processing precedes autonomy expansion (added 2026-07-27, roadmap rule 9):** E09 requires P0-005/006/007 + P0-012 complete (status 2026-09-02: P0-005/006/007 done; **P0-012 done** (PR #33) — the gate is satisfied at the code level; the founder's alert destination (P0-012 step 6) is the last operational piece and should be live before any E09 ticket starts); the E10 outbox is not the bar, but E09 tickets on webhook/cron paths state their idempotency basis.
