...
| # | Ticket | Title | Notes |
|---|---|---|---|
| 1 | PROD-CONFIG-AUDIT | Production configuration audit (docs-only) | **ready.** Cut 2026-09-01. Env read inventory (50 names) → PRESENT/ABSENT/UNKNOWN table + disabled surfaces; includes the D-036 ICP amendment to `ui/design-north-star.md` + `ui/navigation-model.md`. No code. |
| 2 | P0-005A | provider_events retention and pruning | ready (ADR-001 C2). |
| 3 | P0-012 | Monitoring alert delivery and incident hooks | ready. **Q-08 resolved → D-042** (founder Slack ops channel + SMS for SEV-0/1); the seam ships even if the webhook is unconfigured; destination is an outbound webhook, not `lib/slack.ts`. Consumers: `TENANT_SCOPE_VIOLATION`, P0-008 M1, CLEANUP-001 payment notices + reconciliation drift. |
| 3b | CLEANUP-001 | Remove Housecall Pro + Slack approvals (D-052) | **ready.** Cut 2026-09-01. Supersedes P3-001; closes P0-011 TS-6/M1/M2 by removal; test-count accounting required. |
| 3c | UX-001 | Truthful state + polish pass | draft — founder adds Stripe references to `ui/reference-board.md` first. Founder visual review on Preview. |
| 3d | PERF-001 | Response-time audit and fixes | draft — measure → fix → re-measure; additive indexes only; caching/infra = HARD STOP. |
