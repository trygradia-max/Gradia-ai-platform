...
- **P0-004A — booking executor atomicity/concurrency** (carried from the P0-004 review, 2026-08-11; not yet a cut ticket): `executeBookAppointment` lands the lead and external calendar event before the appointments insert, still reports "executed" if that insert fails (loud reconciliation log only — `d43ce16`), and the execution-time re-check → insert window is a check-then-insert race under concurrent approvals. Pre-existing gap, explicitly out of P0-004 scope by review agreement; needs an atomic path or compensation redesign (relates to `08` §4 transaction-boundary standard). Cut as its own ticket before conflict enforcement graduates past *internal*.
