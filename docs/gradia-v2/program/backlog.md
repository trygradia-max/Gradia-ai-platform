...
| # | Ticket | Title | Notes |
|---|---|---|---|
| 1 | P0-011 | Service-role tenant-scoping review and helper design | **Next implementation position as of the 2026-08-28 P0-010 close** — ready; tenancy risk class; proceeds independently of P0-013. Review + `forShop()` design; includes the C-2 shop-binding fix. Its design output feeds E01. The P0-009 soft-ordering precondition is satisfied (P0-009 merged 2026-08-26) — the sweep reviews final `approvals.ts` code and must include the post-P0-009 re-review item recorded in the ticket (`recordPayloadReconciliation` `shop_id` predicate). |
| 2 | P0-012 | Monitoring alert delivery and incident hooks | Delivery seam builds now; final destination config waits on Q-08 (see `blocked.md`). |
| 3 | P0-013 | Production billing model alignment (D-031 three-tier implementation) | **Draft — decision-gated on Q-22** (founder tier decisions; `blocked.md`). **Launch-blocking before live paid billing activation** — `STRIPE_PRICE_*` stays unset in Production (checkout fail-closed, proven at the P0-010 acceptance) until it lands. Payments + database-sensitive risk classes. |

(P0-010 — Production environment and error-surface cleanup — **done 2026-08-28**, merged PR #27 `5d82fa3`, reviewed tree `618cf41`, Cursor APPROVE with one HIGH fixed pre-merge, founder acceptance PASS incl. the recorded production billing exception; left Band 1 — Band 4 items 1–6 shipped with it, with the Builder deviations recorded in its close record. P0-009 — done 2026-08-26, merged PR #25 `d3c0e4d`; close record in `../tickets/P0-009-quote-acceptance-lead-linkage-expiration.md`; its follow-ups are in Band 2 below. P0-008 — done 2026-08-25, PR #23 `1ea198f`; close record and Band 2 follow-ups likewise.)
