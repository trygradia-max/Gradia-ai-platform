...
- **P0-005** (Webhook event idempotency foundation) — now the **next active implementation ticket** (moved up at P0-004A close, 2026-08-11). Database-sensitive: occupies the DB-sensitive slot when it starts. Enters in-progress when the Organizer slots it and a Builder is recorded in `work-in-progress.md`. Not started.

## WIP limits (restated, binding)

| Limit | Rule | Current state |
|---|---|---|
| Active implementation tickets | max **2** | 1 active (P0-001 in-review); P0-002/P0-003/P0-004/P0-004A done (latest: P0-004A, 2026-08-11, PR #15) — one slot free, reserved next for P0-005 ✓ |
| Database-sensitive tickets active | max **1** | **0** — P0-004A shipped its migration (`20260811120000_booking_atomicity.sql`) and closed same-day; slot empty until P0-005 ✓ |
| Payment / tenancy / calendar high-risk tickets active | max **1** | 0 — P0-004A (calendar) released the slot at close (2026-08-11); slot free ✓ |
